  ...
  <div>
    <el-container>
      <div style="margin-bottom: 10px">
        <el-input style="width: 300px" placeholder="请输入合同名称搜索" suffix-icon="el-icon-search" v-model="name"></el-input>
    ...
    <el-table :data="tableData" border stripe header-cell-class-name="headClass" @selection-change="handleSelectionChange" >
      ...
      <el-table-column prop="id" label="合同编号" width="150" align="center" >
      </el-table-column>
      <el-table-column prop="department" label="单位名称" align="center"  >
      </el-table-column>
      <el-table-column prop="name" label="合同名称" align="center" >
      </el-table-column>
      <el-table-column prop="information" label="合同信息" align="center" >
      </el-table-column>
...
export default {
  ...
  methods:{
    load() {
      this.request.get("/document/page", {
        params: {
    ...
    save() {
      this.request.post("/document", this.form).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/document/del/batch", ids).then(res => {
        if (res.code === '200') {
