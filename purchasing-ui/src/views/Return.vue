  ...
  <div>
    <el-container>
      <div style="margin-bottom: 10px">
        <el-input style="width: 300px" placeholder="请输入退货名称搜索" suffix-icon="el-icon-search" v-model="name"></el-input>
    ...
    <el-table :data="tableData" border stripe header-cell-class-name="headClass" @selection-change="handleSelectionChange" >
      ...
      <el-table-column prop="id" label="退货编号" width="100" align="center" >
      </el-table-column>
      <el-table-column prop="name" label="退货名称" align="center" >
      </el-table-column>
      <el-table-column prop="department" label="单位名称" align="center"  >
      </el-table-column>
      <el-table-column prop="price" label="退货金额" align="center" >
      </el-table-column>
      <el-table-column prop="date" label="退货时间"  align="center" >
      </el-table-column>
...
export default {
  ...
  methods:{
    ...
    save() {
      this.request.post("/cancel", this.form).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/cancel/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/cancel/del/batch", ids).then(res => {
        if (res.code === '200') {
