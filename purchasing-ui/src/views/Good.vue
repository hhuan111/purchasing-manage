  ...
  <div>
    <el-container>
      <div style="margin-bottom: 10px">
        <el-input style="width: 300px" placeholder="请输入商品名称搜索" suffix-icon="el-icon-search" v-model="goodname"></el-input>
    ...
    <el-table :data="tableData" border stripe header-cell-class-name="headClass" @selection-change="handleSelectionChange" >
      ...
      <el-table-column prop="price" label="商品单价（元）" align="center"  >
      </el-table-column>
    ...
    <el-dialog title="商品信息" :visible.sync="dialogFormVisible" width="30%">
      <el-form label-width="100px" size="small">
        ...
        <el-form-item label="商品类型">
          <el-select clearable v-model="form.type" placeholder="请选择商品类型" style="width: 100%">
            <el-option v-for="item in types" :key="item.id" :label="item.name" :value="item.name"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="供应商">
          <el-select clearable v-model="form.supplierId" placeholder="请选择供应商" style="width: 100%">
...
export default {
  ...
  methods:{
    load() {
      ...
      this.request.get("/good/goodTypes").then(res => {
        this.types = res.data
      })
